...
/**
 * `core:mavlink` bindings. Expects `app:shared` to provide the application `CoroutineScope` and the [IoDispatcher].
...
val mavlinkModule = module {
    single { ConnectionManager(scope = get(), ioDispatcher = get(IoDispatcher), podStatus = MutableStateFlow(PodStatus.NoPod)) }
}
