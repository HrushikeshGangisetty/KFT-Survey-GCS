...
class MissionGroupsTest {
    ...
    /**
     * Copter, first in the mission, 10 m run-in: takeoff, speed, then per line 6 items (run-in start, photo start,
     * camera on, photo end, camera off, run-out end), then RTL: 2 + 7 × 6 + 1 = 45 items.
     * First line, northbound at x = 15: (15, −10) → (15, 0) → on(20 m, shoot now) → (15, 200) → off → (15, 210).
     */
    @Test
    fun copterSurveyItems() {
        ...
        val line = items.subList(2, 8)
        assertEquals(
            listOf(MissionCommand.WAYPOINT, MissionCommand.WAYPOINT, MissionCommand.DO_SET_CAM_TRIGG_DIST, MissionCommand.WAYPOINT, MissionCommand.DO_SET_CAM_TRIGG_DIST, MissionCommand.WAYPOINT),
            line.map { it.command },
        )
        ...
        assertAt(15.0, 200.0, line[3].position!!)
        assertEquals(0f, line[4].param1, "camera off at the far edge: no photos in the turn")
        assertAt(15.0, 210.0, line[5].position!!)
    ...
    /** Plane: no takeoff (the pilot takes off on the RC), no RTL here, 0 m lead: 1 + 7 × 4 = 29 items. */
    @Test
    ...
    fun groupsFlattenInOrderWithSeqNumbers() {
        ...
        assertEquals(3 + 44, flat.items.size)
    }
