...
**v3 changes:** The GCS never commands flight: arming, takeoff, mode changes, RTL and landing belong to the pilot on the RC (S9). Every MAVLink message is checked against the dialect XML and ArduPilot's own handling (S10). Mission upload/download always treats seq 0 as home (S11). New open item GS-8 (numbered GS-7 in the first v3 draft). The §2.2 and §2.5 rows and the week 2–3 exit check follow from S9.
